...
- The 2-band PCA scatter is driven by the CallSource feature; Web Reported crimes form one band, all other call sources form the other
...
- Ensemble feature selection combining L1 and RF importances for a more robust
  feature subset
- Build a dedicated binary classifier predicting Larceny vs non-Larceny: given that
  Larceny is fundamentally distinct in its temporal and geographic footprint, this
  simpler task could achieve high accuracy and have direct operational value for
  resource allocation
""")
